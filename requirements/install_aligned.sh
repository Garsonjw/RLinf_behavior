...
install_behavior_env() {
    ...
    popd >/dev/null
    uv pip uninstall flash-attn torchaudio || true
    uv pip install ml_dtypes==0.4.1 protobuf==3.20.3
    uv pip install click==8.2.1
    pushd ~ >/dev/null
    uv pip install torch==2.7.1 torchvision==0.22.1
    uv pip install transformers==4.53.2 safetensors==0.5.3 numpy==1.26.4
    popd >/dev/null
}
